        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {SKILLS.map((skillGroup) => (
            <div key={skillGroup.category} className="p-8 rounded-2xl bg-white border border-[#D2B48C]/10 shadow-sm hover:shadow-md transition-shadow group">
              <h3 className=" font-bold text-sm uppercase tracking-widest mb-6 group-hover:text-[#2D2D2D] transition-colors">
                {skillGroup.category}
              </h3>
              <div className="flex flex-wrap gap-2">
                {skillGroup.items.map((skill) => (
                  <span 
                    key={skill} 
                    className="px-3 py-1 bg-[#FDFBF7]  text-sm rounded-full border border-[#D2B48C]/20 hover:bg-[#8B7E66] hover:text-white transition-all cursor-default"
                  >
                    {skill}
                  </span>
                ))}
